...
@dp.table(
    ...
    - raw data is streamed via autoloader from raw volume          
    """
)

def sdp_earthquake_source_data():
    bronze_source = (
        spark.readStream.format("cloudfiles") # cloudfiles uses autoloader to read the data from volume
    ...
    ########### silver_load logic(copied from silver_load) #########
